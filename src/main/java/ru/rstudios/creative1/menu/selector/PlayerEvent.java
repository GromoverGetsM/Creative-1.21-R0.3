...
public class PlayerEvent extends CodingCategoriesMenu {
        ...
        if (event.getCurrentItem() != null) {
            if (selectedCategory == null) {
                ...
                if (category != null) {
                    ...
                    sign.setLine(2, "coding." + category.getType().name().toLowerCase(Locale.ROOT) + "." + category.name().toLowerCase(Locale.ROOT) + ".name");
                    sign.update();
    ...
    private void addRange(List<Byte> list, byte start, byte end) {
