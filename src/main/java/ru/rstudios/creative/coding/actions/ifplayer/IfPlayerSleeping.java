...
public class IfPlayerSleeping extends ActionIf {
    @Override
    public boolean conditionExpression(GameEvent event, List<Entity> selection) {
        for (Entity entity : selection) {
            if (entity instanceof Player player && player.isSleeping()) return true;
        }
        return false;
    }
    ...
    public ActionCategory getCategory() {
        return ActionCategory.IF_PLAYER_SLEEPING;
    }
}
